...
<head>
    ...
    <script src="https://jstest.authorize.net/v1/AuthorizeNet.js"></script>
</head>
...
&lt;/createTransactionRequest&gt;
    ...
    function buildFingerPrint() {
        var url = "https://dpm2.azurewebsites.net/getfingerprint.php"; // Fingerprint Service URL from your Server    
        var deferred = $.Deferred();
        $.get(url, function(responseText) {
            try {
                deferred.resolve(JSON.parse(responseText));
            } catch (e) {
                console.log("fingerprint is not a valid JSON" + responseText)
            }
        });
        return deferred.promise();
    }
    function getSecureData() {
        var promise = buildFingerPrint(); // for accessing fingerprint from server using AJAX call. See More
        promise.then(function(fingerprint) {
            ...
            AuthorizeNet.dispatchData(secureData, 'responseHandler');
        })
    }
